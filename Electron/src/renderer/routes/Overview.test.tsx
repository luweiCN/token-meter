...
const readyPayload: OverviewPayload = {
  ...
  heatmapDays: 371,
  modelRanking: [
    { model: 'claude-fable-5', tokens: 1000, costUsdMicros: 500_000, costUnknownEvents: 0 },
    { model: 'unpriced', tokens: 50, costUsdMicros: 0, costUnknownEvents: 4 }
  ],
  sessionRail: [
    {
      sessionId: 1,
...
describe('Overview (ready)', () => {
  ...
  it('fetches once on mount and renders the KPI row, trend chart, heatmap, ranking and session rail', async () => {
    render(<Overview />);

    await waitFor(() => { expect(api.overview.query).toHaveBeenCalledTimes(1); });

    expect(await screen.findByText('12.35K')).toBeTruthy();                 // KPI 今日 Token
    expect(screen.getByLabelText('Token 用量趋势')).toBeTruthy();           // 趋势图 svg
    expect(document.querySelectorAll('.year-heatmap__cell')).toHaveLength(371); // 热力图 371 格
    const ranking = screen.getByLabelText('模型排行');
    expect(within(ranking).getByText('claude-fable-5')).toBeTruthy();      // 模型排行
    expect(screen.getByText('token-meter')).toBeTruthy();                  // 会话列表
  });

  it('surfaces unknown-cost events so a zero cost is not mistaken for a free one', async () => {
    render(<Overview />);
    // 「部分未知」必须在每一处成本旁都能表达——KPI 今日成本卡与模型排行都各出现一次。
  ...
  it('opens the session rail as an overlay when the badge is clicked', async () => {
    ...
    expect(within(dialog).getByText('token-meter')).toBeTruthy();
  });
