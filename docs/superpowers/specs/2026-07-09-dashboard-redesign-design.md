...
**二、`inode + dev + 尺寸增长` 不能证明"这是追加"。** 原地改写成更大的文件同样满足这三条（`Data.write(to:)` 保留 inode，已实测）。误判为追加的后果是三重的：新内容的前缀永不被解析、旧内容的事件因为走了续读路径而不被 `deleteEvents` 清除、解析器带着旧会话身份继续读。判据必须加上**内容指纹**——文件前 4 KB 的 SHA-256。追加不改前 4 KB，改写必改。本机 19,971 个 codex 文件无一小于 4 KB。
...
- **零事件文件每次扫描都被完整重读。** 跳过条件要求 `usage_events` 里已有该文件的行，而 19,211 个 2026-04-16 之前的 codex 文件永远没有事件（见 §9.3.2），也无法走续读（`shouldResume` 要求尺寸增长）。实测第二遍扫描仍要重读它们、耗时 29.2 s。需要一个能表达"这个文件解析过、结论就是没有事件"的状态。
- **`sawClaudeUsage` 用裸子串匹配 `"usage"`。** 一个没有 sessionId 的辅助文件，只要正文里碰巧出现这个字面量，就会从"跳过"变成"解析失败"，进而把整个 root 拖成 partial。过度保守，不丢数据。
